...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-outline:hover { background: hsl(0 0% 95%); }
    ...
    <div class="wrapper">
        <div class="card">
            <div class="card-header">
                <h1 class="card-title">${msg("rdyResetTitle")}</h1>
                <p class="card-description">${msg("rdyResetDescription")}</p>
            </div>
            ...
            <#if message?has_content && message.type == 'info'>
                <div class="alert alert-info">${kcSanitize(message.summary)}</div>
            </#if>
            ...
            <form class="card-content" action="${url.loginAction}" method="post">
                ...
                <div class="card-footer">
                    <button class="btn" type="submit">${msg("rdySendResetLink")}</button>
                    <a class="btn btn-outline" href="${url.loginUrl}">${msg("rdyBackToLogin")}</a>
                </div>
            </form>
        </div>
